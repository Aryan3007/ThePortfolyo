...
const SideBar = () => {
  ...
  return (
    <div className="lg:h-screen justify-between lg:border-r-2 border-purple-300 h-full lg:fixed flex items-center flex-col z-50 w-full lg:w-96 bg-black text-white">
      <div>
        <h1 className="text-6xl p-4 font-extrabold text-center uppercase">
          {" "}
          {data.user.about.name}
        </h1>
        <div className="flex justify-center w-96 h-64 items-center overflow-hidden rounded-xl">
          <img src={data.user.about?.avatar?.url} className=" w-80" alt="" />
        </div>
      </div>
      ...
      <div className="my-4 gap-4 flex flex-col">
        <div className="flex gap-4">
          <FaLocationDot className="text-purple-300 text-2xl" />
          <h1>{data.user.about.address}</h1>
        </div>
        <div className="flex gap-4">
          <MdEmail className="text-purple-300 text-2xl" />
